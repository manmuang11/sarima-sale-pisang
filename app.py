...
DEFAULT_PRED_FILE = "prediksi_default.xlsx"  # opsional (backup prediksi)


# =========================================================
# CSS LOADER + FALLBACK + BUTTON STYLE
# =========================================================
def load_css():
    css_parts = []

    # theme.css dari repo (kalau ada)
    if THEME_CSS_PATH.exists():
        try:
            css_parts.append(THEME_CSS_PATH.read_text(encoding="utf-8"))
        except Exception:
            pass

    # fallback minimal biar UI tetap enak
    css_parts.append("""
    :root{
      --bg:#FBF7EF;
      --card:#FFFFFF;
    ...
    """)
    # tombol streamlit biar cakep konsisten
    css_parts.append("""
    .stButton > button{
      width:100%;
      border-radius:14px;
...
if LOGO_PATH.exists():
    try:
        logo_html = f"<img src='data:image/png;base64,{img_to_base64(LOGO_PATH)}' style='width:40px;height:40px;object-fit:contain'/>"
    except Exception:
        logo_html = ""
...
# =========================================================
# LOGIN PAGE (NO SCROLL, sidebar hidden ONLY on login)
# =========================================================
def render_login_page():
    # Hide sidebar only for login page (inline CSS, not global class)
    st.markdown(
        """
        <style>
    ...
    st.markdown(
        ...
        .login-title{margin-top:18px;font-size:1.8rem;font-weight:900;color:#111827;text-align:center;}
...
def card(title: str, value: str, sub: str = ""):
    st.markdown(
        f"""
        <div style="background:#fff;border-radius:22px;padding:18px 18px;box-shadow:0 14px 30px rgba(15,23,42,0.08);
...
with st.sidebar:
    st.markdown(
        ...
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;">
          <div style="width:44px;height:44px;border-radius:999px;background:#fff1b8;display:flex;align-items:center;justify-content:center;border:1px solid rgba(0,0,0,.05);">
            {render_logo_fallback()}
          </div>
          <div>
            <div style="font-weight:900;color:#111827;line-height:1.1;">Sale Pisang</div>
            <div style="font-weight:900;color:#111827;line-height:1.1;">Bungo Family</div>
            <div style="color:#6b7280;font-size:.9rem;margin-top:2px;">Dashboard</div>
          </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    st.markdown(
        f"<div style='color:#6b7280;font-size:.92rem;'>Login sebagai: <b>{'Admin' if IS_ADMIN else 'UMKM'}</b></div>",
        unsafe_allow_html=True
    )
...
st.markdown(
    f"""
    <div style="background:#fff;border-radius:28px;padding:18px 18px;box-shadow:0 14px 30px rgba(15,23,42,0.08);border:1px solid rgba(0,0,0,0.03);">
      <div style="display:flex;align-items:center;gap:12px;">
        <div style="width:54px;height:54px;border-radius:999px;background:#fff1b8;display:flex;align-items:center;justify-content:center;border:1px solid rgba(0,0,0,.05);">
          {render_logo_fallback()}
        </div>
        <div>
          <div style="font-size:1.6rem;font-weight:900;color:#111827;">Berapa Pisang yang Perlu Disiapkan?</div>
          <div style="color:#6b7280;margin-top:4px;">Prediksi kebutuhan pisang per bulan (SARIMA)</div>
        </div>
      </div>
    </div>
    """,
...
st.markdown(
    """
    <div style="background:#e6f2ff;border:1px solid rgba(0,0,0,0.04);border-radius:14px;padding:10px 12px;color:#111827;">
...
# =========================================================
# GUARD: kalau belum ada prediksi
# =========================================================
if df_pred_all.empty:
    if IS_ADMIN:
        st.warning(
            "Belum ada hasil prediksi. Admin: buka menu 'Upload & Proses SARIMA' lalu klik Proses. "
...
# =========================================================
    ...
    st.caption("Upload historis → klik Proses SARIMA → hasil masuk dashboard (UMKM tinggal lihat).")
    ...
    if st.button("⚙️ Proses Prediksi & Terapkan", use_container_width=True):
        try:
            with st.spinner("Sedang memproses SARIMA..."):
