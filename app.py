...
# =========================================================
# DEBUG STEP (biar tau nyangkut di mana)
# =========================================================
if "_debug_steps" not in st.session_state:
    st.session_state._debug_steps = True

def STEP(msg: str):
    if st.session_state.get("_debug_steps", False):
        st.write(msg)


# =========================================================
# WRAP ALL APP (biar error muncul di layar)
# =========================================================
try:
    st.success("✅ BOOT OK - mulai jalanin kode utama...")
    STEP("STEP 1: setelah boot")
    ...
    def load_css():
        if THEME_CSS_PATH.exists():
            st.markdown(
                f"<style>{THEME_CSS_PATH.read_text(encoding='utf-8')}</style>",
    ...
    STEP("STEP 2A: sebelum load_css()")
    load_css()
    STEP("STEP 2: setelah CSS")
    ...
    STEP("STEP 3A: sebelum login-check")
    ...
    STEP("STEP 3: setelah login (sudah masuk)")

    # =========================================================
    # DEBUG toggle di sidebar (biar bisa matiin step)
    # =========================================================
    with st.sidebar:
        st.session_state._debug_steps = st.toggle("Debug STEP", value=st.session_state._debug_steps)

    STEP("STEP 5A: sebelum helper bulan/unit/UI")
    ...
    STEP("STEP 5: setelah render sidebar & helpers (aman)")
    ...
    STEP("STEP 3B: sebelum load_hist_only() (baca data_umkm.xlsx)")
    df_hist_default = load_hist_only()
    STEP("STEP 3: setelah load excel (historis)")
    ...
    STEP("STEP 4A: sebelum build aktual/pred default")
    ...
    STEP("STEP 4B: sebelum load_default_pred_file() (baca prediksi_default.xlsx)")
    df_pred_default = load_default_pred_file()
    STEP("STEP 4: setelah preprocess (hist+pred siap)")
    ...
    # SIDEBAR (ROLE LOCK)
    # =========================================================
    STEP("STEP 5B: sebelum render sidebar menu (Beranda/Detail/Upload)")
    with st.sidebar:
        st.markdown(
            f"""
    ...
    STEP("STEP 5: setelah render sidebar")
    ...
    # =========================================================
        ...
        try:
            STEP("STEP 4C: admin preview - sebelum parse_historical_excel(uploaded)")
            df_hist = parse_historical_excel(uploaded)
            STEP("STEP 4D: admin preview - setelah parse_historical_excel(uploaded)")
            st.dataframe(df_hist.head(12), use_container_width=True)
        except Exception as e:
            st.error(f"Gagal baca historis: {e}")
            st.stop()

        STEP("STEP 6: sebelum sarima fit (baru jalan kalau tombol diklik)")
        if st.button("⚙️ Proses Prediksi & Terapkan", use_container_width=True):
            try:
                ...
                STEP("STEP 7: setelah sarima fit")
    ...
    STEP("✅ APP DONE: sampai akhir render")
